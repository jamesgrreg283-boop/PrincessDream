...
If you see **“Stripe price … must be GBP one-time for X pence”** (or the friendlier JSON `stripe_price_mismatch` in the network tab), your `STRIPE_PRICE_*` Price in Stripe does not match the deposit configured in `api/create-checkout-session.mjs` / `api/_lib/packages.mjs`. The response body includes **expected vs actual** amounts. Variable names on Vercel do not validate the ID — each value must be the `price_…` from the **matching** Stripe product.

**Cheap £1 tests (30-minute package):** the code charges **100 pence** for that slug. Your Stripe catalogue may still list **£40** for the thirty-minute product. For test checkouts, **remove** `STRIPE_PRICE_30_MINUTE_APPEARANCE` from Vercel so Checkout uses automatic `price_data` at £1, **or** add a separate **£1** one-time GBP Price in Stripe and paste *that* `price_…` into the env var. Using the existing **£40** Price ID with a site configured for **£1** will always fail validation.
